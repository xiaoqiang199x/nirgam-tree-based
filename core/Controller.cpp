...
Controller<num_ip>::Controller(sc_module_name Controller): sc_module(Controller)
{
    ...
    case ER:
        ...
        break;
    case SINGLE_TREE:
        libname=libname+ string("SINGLETREE_router.so");
        break;
    case DOUBLE_TREE:
        libname=libname+ string("DOUBLETREE_router.so");
        break;
    case MY_TREE:
        libname=libname+ string("MYTREE_router.so");
        break;
    ...
    while (true)
    {
        ...
        for (UI i = 0; i < num_ip; i++)
        {
            ...
            if (rtRequest[i].event() && rtRequest[i].read() == ROUTE)
            {
                ...
                sc_uint<ADDR_SIZE> srcX  = sourceAddressX[i].read();
                sc_uint<ADDR_SIZE> srcY  = sourceAddressY[i].read();
                sc_uint<ADDR_SIZE> dest  = destRequest[i].read();
                sc_uint<ADDR_SIZE> destX = destRequestX[i].read();
                sc_uint<ADDR_SIZE> destY = destRequestY[i].read();
//qrt************************************************************
...
//qrt************************************************************
                ...
                UI op_dir = rtable->calc_next(temp, src, srcX, srcY, dest, destX, destY);
...
///////////////////////////////////////////////////////////////////////////
template<UI num_ip>
void Controller<num_ip>::setTileID(UI id, UI port_N, UI port_S, UI port_E, UI port_W, vector<UI> tree_id)
{
    tileID = id;
    portN = port_N;
    portS = port_S;

...
NWTile<num_nb, num_ic, num_oc>::NWTile(sc_module_name NWTile, UI id, vector<vector<UI> > tree_id): BaseNWTile(NWTile, id, tree_id), vcAlloc("VA"),ctr("Controller")
{
...
//printf("Config IC starts\n");
    ...
    for (UI i = 0; i < num_ic; i++)
    {
        ...
        Ichannel[i]->destRequestX(destReqX[i]);	// dest to Ctr
        Ichannel[i]->destRequestY(destReqY[i]);	// dest to Ctr
        Ichannel[i]->sourceAddress(srcAddr[i]);	// source address to Ctr
        Ichannel[i]->sourceAddressX(srcAddrX[i]);	// source address to Ctr
        Ichannel[i]->sourceAddressY(srcAddrY[i]);	// source address to Ctr
...
//    printf("Config ctr starts\n");
    for (UI i = 0; i < num_ic; i++)
    {
        ...
        ctr.destRequestX[i](destReqX[i]);		// dest address from IC
        ctr.destRequestY[i](destReqY[i]);		// dest address from IC
        ctr.sourceAddress[i](srcAddr[i]);	// source address from IC
        ctr.sourceAddressX[i](srcAddrX[i]);	// source address from IC
        ctr.sourceAddressY[i](srcAddrY[i]);	// source address from IC
...
//////////////////////////////////////////////////////////////////////////
template <int num_nb, int num_ic, int num_oc>
void NWTile<num_nb, num_ic, num_oc>::setID(UI id, vector<vector<UI> > tree_id)
{
    tileID = id;
...
//******************************************************************
        ip->setID(id, priority, tree_id);
//*****************************************************************

    // set id and port mapping for ctr
    ctr.setTileID(id, portN, portS, portE, portW, tree_id[id]);
}

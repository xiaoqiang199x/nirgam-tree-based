...
//					r_vc[(sim_count+NUM_VCS)%NUM_VCS].val.simdata.OCtimestamp = sim_count; //moved to IC
                    ...
                    if((r_vc[(sim_count+NUM_VCS)%NUM_VCS].val.pkthdr.nochdr.flittype == HDT) || (r_vc[(sim_count+NUM_VCS)%NUM_VCS].val.pkthdr.nochdr.flittype == HEAD))
                    {
                        if (r_vc[(sim_count+NUM_VCS)%NUM_VCS].val.simdata.manht_len < r_vc[(sim_count+NUM_VCS)%NUM_VCS].val.simdata.hop_count-1)
                        {
                            cout << "\033[31m";
                            non_optimal_count += 1;
                        }
                        if (LOG >= 19)
                        {
                            cout << "src: " << setw(2) << r_vc[(sim_count+NUM_VCS)%NUM_VCS].val.src;
                            cout << "  dst: " << setw(2) << r_vc[(sim_count+NUM_VCS)%NUM_VCS].val.pkthdr.nochdr.flithdr.header.rthdr.dsthdr.dst;
                            cout << "  srcX: " << setw(4) << r_vc[(sim_count+NUM_VCS)%NUM_VCS].val.srcX;
                            cout << "  dstX: " << setw(4) << r_vc[(sim_count+NUM_VCS)%NUM_VCS].val.pkthdr.nochdr.flithdr.header.rthdr.dsthdr.dstX;
                            if(RT_ALGO == DOUBLE_TREE || RT_ALGO == MY_TREE)
                            {
                                cout << "  srcY: " << setw(4) << r_vc[(sim_count+NUM_VCS)%NUM_VCS].val.srcY;
                                cout << "  dstY: " << setw(4) << r_vc[(sim_count+NUM_VCS)%NUM_VCS].val.pkthdr.nochdr.flithdr.header.rthdr.dsthdr.dstY;
                            }
                            cout << "  Manhattan: " << setw(2) << r_vc[(sim_count+NUM_VCS)%NUM_VCS].val.simdata.manht_len;
                            cout << "  Hop count: " << setw(2) << r_vc[(sim_count+NUM_VCS)%NUM_VCS].val.simdata.hop_count-1 << endl;
                        }
                        cout << "\033[0m";
                        path_len      += (r_vc[(sim_count+NUM_VCS)%NUM_VCS].val.simdata.hop_count - 1);
                        manhattan_len += r_vc[(sim_count+NUM_VCS)%NUM_VCS].val.simdata.manht_len;
                        if (LOG >= 19)
                        {
                            cout << "Total hop count: " << path_len; 
                            cout << "  Total Manhattan distance: " << manhattan_len << endl; 
                        }
                    }

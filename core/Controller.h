...
struct Controller : public sc_module {
	...
	sc_in<sc_uint<ADDR_SIZE> > destRequestX[num_ip];	///< input ports to recieve destination address
	sc_in<sc_uint<ADDR_SIZE> > destRequestY[num_ip];	///< input ports to recieve destination address
	sc_in<sc_uint<ADDR_SIZE> > sourceAddress[num_ip]; ///< input ports to recieve source address
	sc_in<sc_uint<ADDR_SIZE> > sourceAddressX[num_ip]; ///< input ports to recieve source address
	sc_in<sc_uint<ADDR_SIZE> > sourceAddressY[num_ip]; ///< input ports to recieve source address
...
/*	fifo	ctrQ;
	...
	void setTileID(UI tileID, UI portN, UI portS, UI portE, UI portW, vector<UI> tree_id);

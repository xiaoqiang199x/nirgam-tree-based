...
///////////////////////////////////////////////////////////////////////////
void ipcore::setID(UI id,UI priority, vector<vector<UI> > t_id)
{
    tileID = id;
    tree_id = t_id;
...
int ipcore::get_random_dest()
{
    ...
    while ((dest == tileID) || (tree_id[dest][0] == 5))
...
flit* ipcore::create_hdt_flit(int pkt_id, int flit_id, UI route_info)
{
    ...
    flit_out->vcid = 0;
    if((RT_ALGO == SINGLE_TREE) || (RT_ALGO == DOUBLE_TREE) || (RT_ALGO == MY_TREE))
    {
        flit_out->src  = tileID;
        flit_out->srcX = tree_id[tileID][0];
        flit_out->srcY = tree_id[tileID][1];
    }
    else
        flit_out->src = tileID;
    ...
    else if((RT_ALGO == SINGLE_TREE) || (RT_ALGO == DOUBLE_TREE) || (RT_ALGO == MY_TREE))
    {
        flit_out->pkthdr.nochdr.flithdr.header.rthdr.dsthdr.dst  = route_info;
        flit_out->pkthdr.nochdr.flithdr.header.rthdr.dsthdr.dstX = tree_id[route_info][0];
        flit_out->pkthdr.nochdr.flithdr.header.rthdr.dsthdr.dstY = tree_id[route_info][1];
    }
    else
        flit_out->pkthdr.nochdr.flithdr.header.rthdr.dsthdr.dst = route_info;
    ...
    flit_out->simdata.hop_count = 0;
    int src_id_x = tileID / num_cols;
    int src_id_y = tileID % num_cols;
    int dst_id_x = route_info / num_cols;
    int dst_id_y = route_info % num_cols;
    flit_out->simdata.manht_len = abs(src_id_x - dst_id_x) + abs(src_id_y - dst_id_y);

    if(LOG >= 21)
    {
        cout << "Generating flit---src=" << flit_out->src;
        cout << "  srcX=" << flit_out->srcX;
        cout << "  srcY=" << flit_out->srcY << endl;
        cout << "dst=" << flit_out->pkthdr.nochdr.flithdr.header.rthdr.dsthdr.dst;
        cout << "  dstX=" << flit_out->pkthdr.nochdr.flithdr.header.rthdr.dsthdr.dstX;
        cout << "  dstY=" << flit_out->pkthdr.nochdr.flithdr.header.rthdr.dsthdr.dstY;
    }
...
flit* ipcore::create_head_flit(int pkt_id, int flit_id, UI route_info)
{
    ...
    if(LOG >= 21)
    {
        ...
        cout << "dst=" << flit_out->pkthdr.nochdr.flithdr.header.rthdr.dsthdr.dst;
        cout << "  dstX=" << flit_out->pkthdr.nochdr.flithdr.header.rthdr.dsthdr.dstX;
        cout << "  dstY=" << flit_out->pkthdr.nochdr.flithdr.header.rthdr.dsthdr.dstY << endl;
    }

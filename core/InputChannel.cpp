...
void InputChannel<num_op> :: read_flit()
{
    ...
    while (true)
    {
        ...
        if (inport.event())
        {
            ...
            flit_in.simdata.hop_count += 1;
            //cout << "ICtimestamp in IChannel :: "<<	flit_in.simdata.ICtimestamp<<endl;
            if (LOG >= 2)
                eventlog<<"\ntime: "<<sc_time_stamp()<<" name: "<<this->name()<<" tile: "<<tileID<<" cntrl: "<<cntrlID <<" Inport event!"<<" flitID: "<<flit_in<<"current hop count: "<<flit_in.simdata.hop_count;
...
void InputChannel<num_op>::routing_src(flit *flit_in)
{
    ...
    sourceAddressX.write(flit_in->srcX);
    sourceAddressY.write(flit_in->srcY);
...
void InputChannel<num_op>::routing_dst(flit *flit_in)
{
    ...
    destRequestX.write(flit_in->pkthdr.nochdr.flithdr.header.rthdr.dsthdr.dstX);
    destRequestY.write(flit_in->pkthdr.nochdr.flithdr.header.rthdr.dsthdr.dstY);

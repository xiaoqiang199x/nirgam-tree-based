...
struct rt_dst_hdr {
	...
	UI dstX;		///< destination tile ID
	UI dstY;		///< destination tile ID
};
...
struct sim_hdr {     
    ...
    int hop_count;  // count of hops
    int manht_len;  // length of manhattan distance 
...
struct flit {
	...
	UI	 srcX;		///< source tileID
	UI	 srcY;		///< source tileID

	// overloading equality operator
	inline bool operator == (const flit& temp) const {
		if(temp.pkttype != pkttype || temp.simdata.gtime != simdata.gtime || temp.simdata.atime != simdata.atime || temp.simdata.ctime != simdata.ctime || temp.src != src || temp.srcX != srcX || temp.srcY != srcY)
			return false;
		switch(temp.pkttype) {
			case ANT: break;
			case NOC:
